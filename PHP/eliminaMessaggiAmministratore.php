    ...
    if(isset($_SESSION["admin"]) && $_SESSION["admin"] == 1) {
        if(isset($_POST["eliminaMessaggio"])) {
            ...
            if ($connessioneDatabase->esegui($delete) == TRUE) {
                $messaggio = "<p class='msgAmm'>Messaggio eliminato correttamente&period; Potresti vedere ancora il messaggio appena eliminato nella tua lista&comma; eventualmente ricarica la pagina&period;</p>";
                $_SESSION["nuovoMessaggio"] = $messaggio;
                header("location: ../PAGES/messaggiAmministratore.php");
            } else {
                $messaggio = "<p class='msgErrAmm'>ATTENZIONE&excl; Non &egrave; possibile eliminare il messaggio per un problema del database&period; Riprova pi&ugrave; tardi&period;</p>";
                $_SESSION["nuovoMessaggio"] = $messaggio;
                header("location: ../PAGES/messaggiAmministratore.php");
            }
        }
        if(isset($_POST["eliminaConversazione"])) {
            ...
            if ($connessioneDatabase->esegui($deleteR) == TRUE) {
                ...
                if ($connessioneDatabase2->esegui($deleteM) == TRUE) {
                    $messaggio = "<p class='msgAmm'>Conversazione eliminata correttamente&period; Potresti vedere ancora i messaggi appena eliminati nella tua lista&comma; eventualmente ricarica la pagina&period;</p>";
                    $_SESSION["nuovoMessaggio"] = $messaggio;
                    header("location: ../PAGES/messaggiAmministratore.php");
                } else {
                    $messaggio = "<p class='msgErrAmm'>ATTENZIONE&excl; Non &egrave; possibile eliminare la conversazione per un problema del database&period; Riprova pi&ugrave; tardi&period;</p>";
                    $_SESSION["nuovoMessaggio"] = $messaggio;
                    header("location: ../PAGES/messaggiAmministratore.php");
                }
            } else {
